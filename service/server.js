...
module.exports = function(CONFIG, WFCatalogCallback) {
   ...
   */  
  WFCatalog.get(CONFIG.BASE_URL + 'application.wadl', function(req, res, next) {
  ...
  WFCatalog.all(CONFIG.BASE_URL + 'query', function(req, res, next) {
    ...
    if(!Mongo.serverConfig.isConnected()) {
  ...
  WFCatalog.get(CONFIG.BASE_URL + 'query', function(req, res, next) {
    ...
    for(var key in req.query) {
      ...
      if(Array.isArray(value) || req.WFCatalog.query.hasOwnProperty(databaseKey)) {
  ...
  WFCatalog.post(CONFIG.BASE_URL + 'query', function(req, res, next) {
    ...
    req.on('end', function() {
      ...
      if(req.WFCatalog.connected) {
        req.WFCatalog.POSTbody = body; next();
      }
  ...
  WFCatalog.post(CONFIG.BASE_URL + 'query', function(req, res, next) {
    ...
    req.WFCatalog.segments = lines.filter(function(x) {
    ...
    for(var i = 0; i < parameters.length; i++) {
      ...
      // throw 400
      if(!databaseKey) {
        return sendErrorPage(req, res, ERROR.INVALID_PARAMETER_POST, keyValues.key);
      }

      // Options specified as key=value may not be a stream identifier
      if(databaseKey === 'net' || databaseKey === 'sta' || databaseKey === 'loc' || databaseKey === 'cha' || databaseKey === 'ts' || databaseKey === 'te') {
      ...
      if(req.WFCatalog.query.hasOwnProperty(databaseKey)) {
        return sendErrorPage(req, res, ERROR.DOUBLE_PARAMETER_POST, databaseKey);
      }
  ...
  WFCatalog.all(CONFIG.BASE_URL + "query", function(req, res, next) {
    ...
    if(req.WFCatalog.query.include) {
      if(!['default', 'sample', 'header', 'all'].inArray(req.WFCatalog.query.include)) {
        return sendErrorPage(req, res, ERROR.INCLUDE_UNSUPPORTED, req.WFCatalog.query.include);
      }
    ...
    if(req.WFCatalog.query.gran) {
      if(req.WFCatalog.query.gran !== 'day') {
        return sendErrorPage(req, res, ERROR.GRANULARITY_UNSUPPORTED, req.WFCatalog.query.gran);
      }
    ...
    if(req.WFCatalog.query.format) {
      if(req.WFCatalog.query.format !== 'json') {
        return sendErrorPage(req, res, ERROR.FORMAT_UNSUPPORTED, req.WFCatalog.query.format);
      }
    ...
    if(req.WFCatalog.options.minlen !== 0 || req.WFCatalog.options.longestonly) {
  ...
  */
  WFCatalog.all(CONFIG.BASE_URL + "query", function(req, res, next) {
    ...
    for(var key in req.WFCatalog.query) {
      ...
      if(key === "qlt") {
        ...
        if(value !== 'D' && value !== 'R' && value !== 'Q' && value !== 'M' && value !== 'B') {
          return sendErrorPage(req, res, ERROR.QUALITY_INVALID, value);
        }

        req.WFCatalog.parsedQuery['qlt'] = value; continue;
      ...
      if(ext) {
        ...
        req.WFCatalog.parsedQuery[key] = list.length > 1 ? {"$in": list} : list[0];
  ...
  WFCatalog.all(CONFIG.BASE_URL + 'query', function(req, res, next) {
    ...
    function processDailyStream(err, doc) {
      ...
      var cQuery = {
        ...
        'slen': {'$gte': req.WFCatalog.options.minlen}
      }
      ...
      cCursor = Mongo.collection(CONFIG.MONGO.C_COLLECTION).find(cQuery).sort({'slen': 1}).limit(limit);
  ...
  /*
   * Start the WFCatalog webservice
   * Create a single global connection pool for MongoDB
   * set CONFIG.MONGO.POOL_SIZE for a different pool size (default: 10)
   */
    ...
    if(includedP === 'header' || includedP === 'all') {
      schemaDocument['miniseed_header_percentages'] = {
        ...
        'io_and_clock_flags': {
          ...
          'station_volume': doc['io_flags']['svo'],
          'start_time_series': doc['io_flags']['sts'],
          'end_time_series': doc['io_flags']['ets'],
          'clock_locked': doc['io_flags']['clo'],
          'event_in_progress': doc['io_flags']['eip']
        },
        'data_quality_flags': {
          'amplifier_saturation': doc['dq_flags']['asa'],
          'digitizer_clipping': doc['dq_flags']['dic'],
          'spikes': doc['dq_flags']['spi'],
          'glitches': doc['dq_flags']['gli'],
          'missing_padded_data': doc['dq_flags']['mpd'],
          'telemetry_sync_error': doc['dq_flags']['tse'],
          'digital_filter_charging': doc['dq_flags']['dfc'],
          'suspect_time_tag': doc['dq_flags']['stt']
        },
        'activity_flags': {
          'calibration_signal': doc['ac_flags']['cas'],
          'time_correction_applied': doc['ac_flags']['tca'],
          'event_begin': doc['ac_flags']['evb'],
          'event_end': doc['ac_flags']['eve'],
          'positive_leap': doc['ac_flags']['pol'],
          'negative_leap': doc['ac_flags']['nel']
        }
      }
    }
  ...
  function parseSegmentQuery(req) {
    ...
    for(var key in segment) {
      ...
      var list = segment[key].split(",").map(function(x) {
        return replaceWildcards(x);
      }).filter(function(x) {
        return x !== null;
      });
  ...
  function sendErrorPage(req, res, error, extra) {
    ...
    var errorStatus = 'Error ' + res.statusCode + ': ' + ERROR.STATUS_CODES[error.code]
  ...
  function setMongoAuthentication() {
    ...
    if(CONFIG.MONGO.AUTHENTICATE) {
      CONFIG.MONGO.AUTHHOST = 'mongodb://' + CONFIG.MONGO.USER + ':' + CONFIG.MONGO.PASS + '@' + CONFIG.MONGO.HOST + '?authMechanism=SCRAM-SHA-1';
    } else {
...
if(require.main === module) {
  ...
  new module.exports(CONFIG, function() {
    console.log("Single WFCatalog has been started on " + CONFIG.HOST + ":" + CONFIG.PORT)
  });
